...
const Dashboard: React.FC = () => {
  const { markets, isLedgerReady } = useStore();
  const [filter, setFilter] = React.useState('ALL');
  const [search, setSearch] = React.useState('');

  const filteredMarkets = useMemo(() => {
    return markets.filter(m => {
      const matchesSearch = m.payload.title.toLowerCase().includes(search.toLowerCase());
      const matchesFilter = filter === 'ALL' || m.payload.category === filter;
      ...
      return (
          <div className="min-h-[50vh] flex items-center justify-center">
              <div className="text-center space-y-4">
                  ...
                  <p className="text-slate-400">Syncing with Global Synchronizer...</p>
              </div>
          </div>
      )
  }
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-slate-900 via-slate-900 to-indigo-950 border border-slate-800 p-8 md:p-12 shadow-2xl">
        <div className="relative z-10 max-w-2xl space-y-6">
            <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight leading-tight">
                Canton Network <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Prediction Protocol</span>
            </h1>
            <p className="text-slate-400 text-lg leading-relaxed max-w-lg">
                Decentralized markets running on Daml smart contracts. Atomic settlement, verified oracles, and privacy-enabled trading.
            </p>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-slate-100 font-bold text-xl">
                ...
                <h2>Active Contracts (ACS)</h2>
            </div>
            <span className="text-slate-500 text-sm">{filteredMarkets.length} contracts</span>
        </div>
        ...
        {filteredMarkets.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-slate-800 rounded-3xl bg-slate-900/50">
            ...
            <h3 className="text-slate-400 font-bold text-lg">No markets found in ACS</h3>
            <p className="text-slate-600">Querying Participant Query Store (PQS)...</p>
          </div>
        ) : (
